import { signInWithGoogle, signInWithKakao } from '@/actions/auth';

export default function LoginPage() {
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ backgroundColor: '#fdf6f7' }}>
      <div className="w-full max-w-sm p-8 bg-white rounded-2xl shadow-sm space-y-4">
        <div className="text-center mb-6">
          <h1 className="text-2xl font-bold" style={{ color: '#800020' }}>메노아</h1>
          <p className="text-sm text-gray-500 mt-1">갱년기 건강 파트너</p>
        </div>

        {/* Google */}
        <form action={signInWithGoogle}>
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-3 px-4 py-3 border border-gray-300 rounded-xl text-sm font-medium hover:bg-gray-50 transition-colors"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24">
              <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"/>
              <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"/>
              <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"/>
              <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"/>
            </svg>
            Google로 시작하기
          </button>
        </form>

        {/* Kakao */}
        <form action={signInWithKakao}>
          <button
            type="submit"
            className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-[#FEE500] rounded-xl text-sm font-medium text-[#191919] hover:bg-[#fdd800] transition-colors"
          >
            <svg className="w-5 h-5" viewBox="0 0 24 24" fill="#191919">
              <path d="M12 3C6.477 3 2 6.477 2 10.5c0 2.628 1.607 4.937 4.037 6.31-.168.615-.606 2.22-.695 2.565-.109.432.159.426.333.31.137-.09 2.173-1.473 3.052-2.076.41.057.83.087 1.273.087 5.523 0 10-3.477 10-7.5S17.523 3 12 3z"/>
            </svg>
            카카오로 시작하기
          </button>
        </form>

        {/* Naver */}
        <a
          href="/auth/naver"
          className="w-full flex items-center justify-center gap-3 px-4 py-3 bg-[#03C75A] rounded-xl text-sm font-medium text-white hover:bg-[#02b351] transition-colors"
        >
          <svg className="w-5 h-5" viewBox="0 0 24 24" fill="white">
            <path d="M16.273 12.845L7.376 0H0v24h7.727V11.155L16.624 24H24V0h-7.727z"/>
          </svg>
          네이버로 시작하기
        </a>
      </div>
    </div>
  );
}
