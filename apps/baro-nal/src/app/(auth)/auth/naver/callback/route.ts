...
export async function GET(request: Request) {
  ...
  try {
    ...
    if (!naver?.email) return NextResponse.redirect(`${origin}/login?error=naver_no_email`);

    const { email, name = null, profile_image: avatar = null } = naver;
    const admin = createServiceRoleClient();

    const [existingUser] = await db
      .select({ supabase_id: br045_nal_users.supabase_id })
      .from(br045_nal_users)
      .where(eq(br045_nal_users.email, email))
      .limit(1);

    let supabaseUserId: string;
    if (existingUser) {
      supabaseUserId = existingUser.supabase_id;
    ...
    await db
      ...
      .values({ supabase_id: supabaseUserId, email, name, avatar_url: avatar })
      .onConflictDoUpdate({
        target: br045_nal_users.supabase_id,
        set: { email, name, avatar_url: avatar, updated_at: new Date() },
      });
    ...
    if (!actionLink) return NextResponse.redirect(`${origin}/login?error=naver_failed`);
